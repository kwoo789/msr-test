...
function refreshFrs(){		
       ...
       top.bottomFrame.location.href = 'varsub-repoasset-page.dsp?projectName=%value -htmlcode projectName%&mapSetName=%value -htmlcode deploymentMapName%&runtimeType=%value -htmlcode runtimeType%&serverAliasName=%value -htmlcode serverAliasName%&assetCompositeName=%value -htmlcode assetCompositeName%&assetID=%value -htmlcode assetID%';
...
function refreshPropertiesPage(runtimeAndServeralias){
   ...
   for (var i = 0; i < allElements.length; i++) {
     ...
     if(is_csrf_guard_enabled && needToInsertToken) {
     	 top.bottomFrame.location.href = 'varsub-repoasset-page.dsp?projectName=%value -htmlcode projectName%&mapSetName=%value -htmlcode mapSetName%&runtimeAndServeralias='+runtimeAndServeralias+'&assetCompositeName=%value -htmlcode assetCompositeName%&assetID=%value -htmlcode assetID%&componentName='+cName+'&' + _csrfTokenNm_ + '=' + _csrfTokenVal_;
     } 
     else {
     	 top.bottomFrame.location.href = 'varsub-repoasset-page.dsp?projectName=%value -htmlcode projectName%&mapSetName=%value -htmlcode mapSetName%&runtimeAndServeralias='+runtimeAndServeralias+'&assetCompositeName=%value -htmlcode assetCompositeName%&assetID=%value -htmlcode assetID%&componentName='+cName;     		
   	 }
}
</SCRIPT>
...
<TABLE class="tableView" width="100%">
	...
	<TR>
		<INPUT type="hidden" name="assetCompositeDisplayName" value='%value assetCompositeName%'>	
		<INPUT type="hidden" name="assetCompositeName" value='%value assetID%'>		
